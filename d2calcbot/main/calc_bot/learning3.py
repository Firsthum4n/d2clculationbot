...
from main.calc_bot.test_data3 import matches_test_3, matches_result_3
import json
import torch
...
filepath = 'main/calc_bot/data3.json'
data = []
...
model = MainNetwork()
model.load_state_dict(torch.load('main/calc_bot/actual_models/dota_model_ver002.pth'))
...
optimizer = torch.optim.AdamW(model.parameters(), lr=0.00000015 , weight_decay=0.0000001)
...
torch.save(model.state_dict(), 'main/calc_bot/actual_models/dota_model_ver005.pth')

model = MainNetwork()
model.load_state_dict(torch.load('main/calc_bot/actual_models/dota_model_ver005.pth'))
